...
export default function AdminCategoriesPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError('Khong the tai categories.')
    } finally {
      setLoading(false)
    }
  }
  ...
  async function remove(id: number) {
    if (!confirm('Deactivate category nay?')) return
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-100">
        <h1 className="text-2xl font-black text-slate-950">Category management</h1>
      ...
      <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-5">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          <input value={form.categoryName} onChange={(e) => setForm((p) => ({ ...p, categoryName: e.target.value }))} placeholder="Category name" className="h-11 rounded-xl border border-slate-200 px-3 text-sm font-semibold" />
          ...
          <textarea value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} placeholder="Description" className="rounded-xl border border-slate-200 px-3 py-3 text-sm font-semibold md:col-span-2" />
        ...
        <div className="mt-4 flex gap-2">
          <button className="inline-flex h-11 items-center gap-2 rounded-xl bg-indigo-600 px-5 text-xs font-black uppercase tracking-widest text-white"><Plus className="h-4 w-4" /> {form.id ? 'Cap nhat' : 'Tao category'}</button>
      ...
      {loading ? <StateBox text="Dang tai..." /> : error ? <StateBox text={error} /> : categories.length === 0 ? <StateBox text="Chua co category." /> : (
          ...
          {categories.map((category) => (
            <div key={category.id} className="grid gap-3 border-b border-slate-100 p-4 last:border-0 md:grid-cols-[1fr_auto] md:items-center">
              <div>
                ...
                <p className="text-sm font-semibold text-slate-500">Sort {category.sortOrder} / {category.isActive ? 'Active' : 'Inactive'}</p>
              </div>
