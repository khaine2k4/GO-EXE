...
const STATUS_BADGE: Record<string, string> = {
  ...
  AWAITING_CUSTOMER: 'border-cyan-200 bg-cyan-50 text-cyan-700',
...
const STATUS_LABEL: Record<string, string> = {
  ...
  AWAITING_CUSTOMER: 'Chờ khách duyệt',
...
export default function BookingManager({ initialBooking, onChanged }: { initialBooking?: BookingDto | null; onChanged?: () => void }) {
  ...
  const handleReject = async (id: number) => {
    setDialog({
      ...
      onConfirm: async (reason) => {
        ...
        setActionLoadingId(id)
        try {
          const updated = await rejectBooking(id, reason || undefined)
          toast.push({
            type: 'success',
            title: 'Đã từ chối',
  ...
  return (
    <div className="space-y-6 pb-12 animate-in fade-in duration-500">
      ...
      <Drawer title={selected ? `Đơn Đặt Lịch #${selected.bookingCode}` : 'Chi tiết đơn đặt'} open={Boolean(selected)} onClose={() => setSelected(null)}>
        {selected && (
          <div className="space-y-5">
            <div className="rounded-2xl bg-slate-50 p-4 border border-slate-100">
              <span className={`rounded-xl px-3 py-1.5 text-xs font-black uppercase ${STATUS_BADGE[selected.status] ?? 'bg-slate-100 text-slate-600'}`}>
                {STATUS_LABEL[selected.status] ?? selected.status}
              </span>
              <h3 className="mt-4 text-lg font-black text-slate-950 leading-snug">{selected.customerName}</h3>
              <p className="mt-1 text-sm font-semibold text-slate-500">{selected.packageName}</p>
            </div>
            <Detail label="Lịch Chụp" value={`${formatDate(selected.shootingDate)} lúc ${selected.startTime} - ${selected.endTime}`} />
            <Detail label="Địa Điểm" value={selected.shootingLocation || 'Tại Studio'} />
            <Detail label="Tổng Tiền" value={formatVnd(selected.totalPrice)} />
            <Detail label="Phí Dịch Vụ" value={formatVnd(selected.commissionAmount)} />
            <Detail label="Doanh Thu Của Bạn" value={formatVnd(selected.studioRevenue)} />
            <Detail label="Ngày Tạo Đơn" value={formatDateTime(selected.createdAt)} />
            {selected.note && (
              <div className="rounded-xl border border-slate-200 p-4">
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-1">Ghi chú của khách:</span>
                <p className="text-sm font-semibold text-slate-600 leading-relaxed">{selected.note}</p>
              </div>
            )}
            <div className="flex flex-wrap gap-2 border-t border-slate-100 pt-5">
              {selected.status === 'PENDING_CONFIRMATION' && (
                <>
                  <button disabled={actionLoadingId === selected.id} type="button" onClick={() => handleConfirm(selected.id)} className="inline-flex h-10 items-center gap-2 rounded-xl bg-slate-950 hover:bg-slate-800 px-4 text-xs font-black uppercase text-white shadow-md transition active:scale-95">
                    <CheckCircle2 className="h-4 w-4" /> Xác nhận đơn
                  </button>
                  <button disabled={actionLoadingId === selected.id} type="button" onClick={() => handleReject(selected.id)} className="inline-flex h-10 items-center gap-2 rounded-xl border border-rose-200 hover:bg-rose-50 px-4 text-xs font-black uppercase text-rose-600 transition active:scale-95">
                    <XCircle className="h-4 w-4" /> Từ chối nhận
                  </button>
                </>
              )}
              {selected.status === 'CONFIRMED' && (
                <button disabled={actionLoadingId === selected.id} type="button" onClick={() => handleStart(selected.id)} className="h-10 rounded-xl bg-indigo-600 hover:bg-indigo-700 px-4 text-xs font-black uppercase text-white shadow-md transition active:scale-95">
                  Bắt đầu chụp
                </button>
              )}
              ...
              {selected.status === 'AWAITING_CUSTOMER' && (
                <span className="rounded-xl bg-cyan-50 border border-cyan-200 px-4 py-3 text-xs font-black uppercase text-cyan-700 w-full text-center">
                  ⏳ Đang chờ khách hàng xác nhận hoàn thành
                </span>
              )}
            </div>
          </div>
